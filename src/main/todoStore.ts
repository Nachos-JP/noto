...
export const normalizeAppState = (value: unknown): AppState => {
  ...
  return {
    todos: Array.isArray(value.todos) ? value.todos.map(normalizeTodo).filter((todo): todo is Todo => todo !== null) : [],
    viewMode: isViewMode(value.viewMode) ? value.viewMode : fallback.viewMode,
    searchQuery: typeof value.searchQuery === 'string' ? value.searchQuery : fallback.searchQuery,
    statusFilter:
...
const isViewMode = (value: unknown): value is ViewMode => value === 'list' || value === 'kanban'

const isSortBy = (value: unknown): value is AppState['sortBy'] =>
  value === 'updatedAt' || value === 'createdAt' || value === 'dueDate' || value === 'priority'
