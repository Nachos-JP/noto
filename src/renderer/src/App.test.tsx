...
const installApiMock = (state: AppState = createEmptyAppState()) => {
  const api = {
    ...
    saveState: vi
      .fn()
      .mockImplementation((nextState: AppState) => Promise.resolve(nextState)),
  };
...
describe("App", () => {
  ...
  it("creates a task and persists the next state", async () => {
    ...
    expect(
      await screen.findByText("Write component tests"),
    ).toBeInTheDocument();
  ...
  it("filters visible tasks by search text", async () => {
    ...
    await user.type(
      screen.getByPlaceholderText("Search title, note, or tag"),
      "coffee",
    );
  ...
  it("switches to the kanban board and moves a task", async () => {
    ...
    await user.click(
      within(task as HTMLElement).getByRole("button", {name: "In Progress"}),
    );
