...
describe('todoLogic', () => {
  ...
  it('sets completion timestamp only for done tasks', () => {
    const done = updateTodoStatus(makeTodo({}), 'done', '2026-07-08T12:00:00.000Z')
    const reopened = updateTodoStatus(done, 'inProgress', '2026-07-08T13:00:00.000Z')
  ...
  it('filters by text, status, and priority', () => {
    const todos = [
      makeTodo({ id: '1', title: 'Buy parts', status: 'todo', priority: 'low', tags: ['errand'] }),
      makeTodo({ id: '2', title: 'Ship release', status: 'done', priority: 'high', tags: ['work'] })
    ]
