...
export const updateTodo = (
  todo: Todo,
  updates: Partial<Pick<Todo, 'title' | 'description' | 'priority' | 'dueDate' | 'tags'>>,
  now = new Date().toISOString()
): Todo => ({
  ...todo,
  ...updates,
...
export const filterAndSortTodos = (todos: Todo[], filters: TodoFilters): Todo[] => {
  ...
  return [...todos]
    .filter((todo) => {
      const matchesQuery =
        query.length === 0 ||
        [todo.title, todo.description, ...todo.tags].some((value) => value.toLowerCase().includes(query))
      const matchesStatus = filters.statusFilter === 'all' || todo.status === filters.statusFilter
      const matchesPriority = filters.priorityFilter === 'all' || todo.priority === filters.priorityFilter
...
const compareTodos = (a: Todo, b: Todo, sortBy: AppState['sortBy']): number => {
  ...
  if (sortBy === 'dueDate') {
    return compareNullableDate(a.dueDate, b.dueDate) || b.updatedAt.localeCompare(a.updatedAt)
  }
